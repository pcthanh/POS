...
namespace ServicePOS
{
    public class PrinterSettingServer : IPrinterSettingServer
    {
        ...
        public int checkExits(int Category, int ProductID, int Printer)
        {
            int result = 0;
            try
            {
                var count = _context.PRINTE_JOB_DETAIL.Where(x => x.CategoryID == Category && x.ProductID == ProductID && x.PrinterID == Printer).Count();
                result = count;
            }
            catch (Exception ex)
            {
        ...
        public int DeletePrinter(PrinterModel priter)
        {
            int result = 0;
            try
            {
                string delete = "DELETE FROM PRINTER WHERE ID='" + priter.ID + "'";
                result = _context.Database.ExecuteSqlCommand(delete);
            }
            catch (Exception ex)
            {
                LogPOS.WriteLog("PrinterSettingServer:::::::::::::::::DeletePrinter::::::::::::::::::" + ex.StackTrace);
            }
            return result;
        }
    }
